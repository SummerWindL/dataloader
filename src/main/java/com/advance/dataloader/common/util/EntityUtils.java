...
import com.advance.dataloader.common.base.BaseOracleSqlModel;
...
public class EntityUtils {
     ...
     * @return T
     */
    public static <T> T baseModel2Clz(BaseOracleSqlModel model, Class<T> clz) {
        if (!ObjectUtils.isEmpty(model)) {
            String jsonstr = model.getRetvalue();
            if (null != jsonstr) {
                T t = null;
                try {
                    t = jsonAdaptor.readValue(jsonstr, clz);
//                    CollectionType collectionType = jsonAdaptor.getTypeFactory().constructCollectionType(List.class, clz);
//                    t = jsonAdaptor.readValue(SerializationDefine.StrArr2Str(jsonstr), collectionType);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                return t;
     ...
     * @return T
     */
    public static <T> T baseModel2Clz(List<Object[]> list, Class<T> clz) {
        if (!CollectionUtils.isEmpty(list)) {
            String[] jsonstr = SerializationDefine.List2StringArray(list);
            if (null != jsonstr) {
                T t = null;
                try {
                    t = jsonAdaptor.readValue(SerializationDefine.List2Str(list), clz);
//                    CollectionType collectionType = jsonAdaptor.getTypeFactory().constructCollectionType(List.class, clz);
//                    t = jsonAdaptor.readValue(SerializationDefine.StrArr2Str(jsonstr), collectionType);
                } catch (IOException e) {
                    e.printStackTrace();
